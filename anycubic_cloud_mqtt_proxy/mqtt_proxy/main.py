...
class ProxyService:
    ...
    def _publish_ha_discovery_for_printer(self, pinfo: dict[str, Any]) -> None:
        ...
        try:
            LOG.info("Publicando discovery HA (sensor status): %s", topic)
        ...
        try:
            LOG.info("Publicando discovery HA (sensor online): %s", online_topic)
            self.local_client.publish(online_topic, json.dumps(payload_online), qos=0, retain=True)
        except Exception as e:
            LOG.error("Falha ao publicar discovery HA online para %s: %s", key, e)
    ...
    def _publish_printer_online(self, printer_key: str) -> None:
        ...
        try:
            ...
            pass
        try:
            LOG.info("Atualizando online da impressora %s: %s", printer_key, online_val)
            self.local_client.publish(topic, online_val, qos=0, retain=True)
        except Exception as e:
            LOG.error("Falha ao publicar online %s para %s: %s", online_val, printer_key, e)
